...
void im2col_cpu(const Dtype *im, const int channels, const int height, const int width, const int kernel_h,
...
template<typename Dtype>
void col2im_cpu(const Dtype *col, const int channels, const int height, const int width, const int kernel_h,
                const int kernel_w, const int pad_h, const int pad_w, const int stride_h, const int stride_w,
                Dtype *im) {
    bf_set(channels*height*width, Dtype(0),im);
    const int col_h = (height + 2 * pad_h - kernel_h) / stride_h + 1;
    const int col_w = (width + 2 * pad_w - kernel_w) / stride_w + 1;
    const int col_c = (channels*kernel_h*kernel_w);
    for (int c = 0; c < col_c; c++){
